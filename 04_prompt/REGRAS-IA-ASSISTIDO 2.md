...
## Escopo atual: Aula 10, DOM e eventos

As telas passam a **reagir**. O JavaScript lê o que foi digitado, calcula e
escreve o resultado na página. Ainda não guarda nada ao recarregar.
...
- **`innerHTML`**: use `textContent`. Com `innerHTML`, texto digitado por
  alguém pode virar código rodando na página. É decisão de segurança, não de estilo
- **`onclick` e afins escritos no HTML**: o comportamento fica no `.js`
