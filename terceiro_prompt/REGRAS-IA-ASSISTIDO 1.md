...
## Escopo atual: Aula 8, layout que se adapta

As telas ganham **layout**: os elementos se organizam no espaço, começando
pelo celular. Continua sem comportamento: nada de JavaScript.
...
### 6. Regra extra desta aula: eu preciso prever antes de rodar

Layout tem resultado visível, então dá para testar se eu entendi.
Antes de salvar o CSS que a IA gerou:

1. Leio o código linha por linha.
2. **Digo em voz alta o que vai aparecer na tela.**
3. Rodo e confiro.
...
### Regras de comentário — formato e exemplos

Use estes formatos para que os comentários ajudem o aluno a entender decisões, preencher lacunas e justificar alterações.

- **POR QUE:** explique por que foi escolhida esta tag/estrutura (não repita o óbvio).
...
- **DECISAO SUA:** marque decisões que o aluno precisa confirmar (não escolha em silêncio).

```html
<!-- DECISAO SUA: esta informação é título ou apoio? Se for título, troque para h2. -->
<p>...</p>
```

- **COMPLETAR:** deixe lacunas centrais para o aluno preencher à mão (mínimo três itens quando for lista).

```html
<!-- COMPLETAR: escrever aqui a lista dos itens do pedido, usando ul e li -->
```

- **EU:** o aluno usa este comentário para registrar o que mudou e por quê.

```html
<!-- EU: troquei para h2 porque na minha tela isto é subtítulo da seção, não título principal -->
```

Para CSS, use comentários em bloco com o mesmo propósito:

```css
/* POR QUE: .topo usa flex para organizar marca e menu; no mobile fica em coluna para leitura linear */
```

Observação: comentários devem justificar escolhas e orientar. Evite comentários que só repitam a tag ou descrevam o óbvio.
